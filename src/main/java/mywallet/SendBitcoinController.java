...
import mywalletK.helper.WalletUtils;
...
public class SendBitcoinController implements Initializable {
    ...
    private void sendBitcoin(String strDest, String strValue, String strMemo)
        ...
        if (wallet.isEncrypted()) {
            Optional<String> password = DialogBuilder.buildPasswordInputDialog(null).showAndWait();
            if (!password.isEmpty()) {
                request.aesKey = wallet.getKeyCrypter().deriveKey(password.get());
            }
        }
        request.tx.setPurpose(Transaction.Purpose.USER_PAYMENT);

        request.feePerKb = Coin.valueOf(10000);
        SendResult result = wallet.sendCoins(request);
        WalletUtils.trasactionConfidence(result.tx.getConfidence());
